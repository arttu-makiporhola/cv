...
const GitHubRepoItem: React.FC<GitHubRepoItemProps> = ({
    title,
    url,
    forkedFromUrl,
    description,
    ...
    return (
        <div className='shadow-xl rounded-lg border-1 overflow-hidden print:border-0 print:shadow-none'>
            <div className='hidden print:block'>
                <a href={url} target='_blank' rel='noreferrer' className='font-bold'>{url}</a>
            </div>
            {siteBanner &&
                <div className='relative rounded-t-lg overflow-hidden print:hidden bg-[#000] h-40'>
                    {siteBanner}
                </div>
            }
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="px-2 py-0.5 text-xs font-medium border rounded-full">
                            {t('projectsCard.git.public') || 'Public'}
                        </span>
                    </div>
                    ...
                    {forkedFromUrl && (
                        <p className="text-xs mt-[-4px]">
                            Forked from <a href={forkedFromUrl} className="hover:underline" rel='noreferrer'>{forkedFromUrl.split('/').slice(-2).join('/')}</a>
                        </p>
                    )}
                        ...
                        <div className="flex flex-wrap gap-1 mt-2">
                            {tags.map((tag) => (
                                <a
                                    ...
                                    className="px-2.5 py-0.5 text-xs font-medium bg-[#388bfd1a] hover:bg-[#1f6feb] hover:text-white rounded-full transition-colors"
                                >
                                    {tag}
                                </a>
                            ))}
                    ...
                    <div className="flex flex-wrap items-center gap-4 mt-4 text-xs">
                        ...
                        <span>
                            ...
                            <span> </span>
                            <TimeText time={lastUpdated} options={{
                                month: 'short',
                                day: 'numeric',
                                year: 'numeric',
                            }} />
