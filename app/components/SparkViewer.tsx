...
export default function SparkViewer({
  ...
  useEffect(() => {
    ...
    (async () => {
      const [THREE, { SplatMesh }, { OrbitControls }] = await Promise.all([
        import('three'),
      ...
      const defaultSpherical = new THREE.Spherical().setFromVector3(
        camera.position.clone().sub(orbitControlsObj.target)
      );
      ...
      const lerpSpherical = (
        a: InstanceType<typeof THREE.Spherical>,
        b: InstanceType<typeof THREE.Spherical>,
        t: number
      ) => {
      ...
      const animate = () => {
        ...
        if (!isInteracting && now >= returnStartTime) {
          ...
          if (!returnStartSpherical) {
            returnStartSpherical = new THREE.Spherical().setFromVector3(
              camera.position.clone().sub(orbitControlsObj.target)
            );
          }
        ...
        const isReturning = !isInteracting && now >= returnStartTime && returnStartSpherical &&
        ...
        if (isWobbling && !isInteracting && !isReturning) {
          ...
          if (now >= wobbleStartTime) {
            ...
            const wobbleSpherical = new THREE.Spherical(
              defaultSpherical.radius,
              defaultSpherical.phi + Math.sin(angle) * wobbleAmplitude,
              defaultSpherical.theta + Math.cos(angle) * wobbleAmplitude
            );
  ...
  return (
    <div
      ref={containerRef}
      className={`h-full w-full bg-transparent ${isLoaded ? 'cursor-pointer' : ''} ${className}`}
